...
function BankQuestion(props) {
    const {reload} = props;
    const [editReload, setEditReload] = useState(false);
    const [giaiTich, setgiaiTich] = useState([]);
    ...
    const handleSubmit = () => {
        ...
        const correctCount = answers.reduce((score, answer, idx) => {
            if(answer != giaiTich[idx].answerCorrect) {
                let a = giaiTich[idx].answerCorrect;
                console.log(a);
                answerIncorrect += `Câu ${idx + 1} sai, đáp án đúng là: ${array[a].name}\n`;
                console.log(`Câu ${idx + 1} sai, đáp án đúng là: ${array[a].name}`);
            }
            
            return answer == giaiTich[idx].answerCorrect ? score + 1 : score;
        }, 0);
        alert(`Bạn trả lời đúng ${correctCount}/${giaiTich.length} câu.\n ${answerIncorrect}`);
    };
    let i=1;
    return (
        <>
            ...
            <MathJaxContext version={3} config={config}>
                {giaiTich.map((item, index) => (
                    <div className='question' key={index}>Question {i++}:
                        ...
                        <EditQuestion item= {item} onReload = {handleReload}/>
                        <DeleteQuestion item ={item} onReload = {handleReload}/>
                    </div>
