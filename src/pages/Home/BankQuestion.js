...
function BankQuestion() {
    ...
    const handleSelect = (index, index1) => {
        ...
        const updatedDaiSo = [...daiSo];
        console.log(updatedDaiSo);
        updatedDaiSo[index].selectedOption = index1; // Lưu đáp án đã chọn
        setDaiSo(updatedDaiSo);
    };
    return (
        <>
            <h1>Đại số</h1>
            <MathJaxContext version={3} config={config}>
                {daiSo.map((item, index) => (
                    <div key={index}>Question {item.id}:
